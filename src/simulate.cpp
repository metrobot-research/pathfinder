...
std::vector<Vec> simulate(Path path, Vec start_pos, Vec start_heading, double speed) {
  std::vector<Vec> locs;
  Vec pos = start_pos;
  Vec heading = start_heading.unit();
  ...
  while (i < 20) {
    ...
    std::cout << "---\n(x-" << center.x() << ")^2 + (y-" << center.y()
              << ")^2 = " << radius * radius << "\n";
    std::cout << "speed: " << speed << " radius: " << radius << "\n";
    std::cout << "circle heading: " << circle_heading << "\n";
    std::cout << "unit tangent: " << circle.unit_tangent_at(pos) << "\n";
    std::cout << "unit tangent dot: " << circle.unit_tangent_at(pos).dot(circle_heading) << "\n";
